...
if (__isPTT) {
  ...
  // M-Team 只有种子浏览列表页(/browse/*)需要瀑布流, 其余页面(/message/*、
  // /usercp、/profile、首页、详情页等)都不需要。改为白名单判定: 仅
  // /browse/ 开头才挂载。虽然 userscript.config.js exclude 已拦截一部分,
  // 但 SPA 客户端路由跳转时脚本不会被重载, 需在运行时也判断路径。
  if (!/\/browse\//i.test(window.location.pathname)) {
    console.log("M-Team 非浏览列表页(仅 /browse/ 需瀑布流): 跳过瀑布流挂载");
  } else {
  let mtTries = 0;
